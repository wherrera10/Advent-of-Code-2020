input = """
fvxzl nfcrh kscdhn khnqq jkvcbf jhlvrg chc fk zbh hgrptqb zrvtg tdhv jhqpdf jvjhx rrmlg mxsklfx vzn tmp hrhg fpcstf zgpv pdpgm cbsvx ttkn bkmgx xknj bxfvt bdccss tqjg mkpmkx fhpm kcgtlb ktrkfn schkf gcbbnqb sfb rjnx hcms cjt qhhsj qddlrq flnhl vxzpfp tplrt bnl ddmg mxbbst (contains shellfish, sesame)
...
const pairs = Pair{Vector{String}, Vector{String}}[]
const sdict = Dict{String, String}()
const lines = split(input, "\n")
for line in lines
    words = filter(x -> length(x) > 1, String.(split(line, r"[\,\s\)]+")))
    idx = findfirst(w -> w[1] == '(', words)
    if idx == nothing
        push!(pairs, words => String[])
    else
        push!(pairs, words[1:idx-1] => words[idx+1:end])
    end
end
...
function part1()
    ...
    for (ingredients, allergens) in pairs
        ...
        for a in allergens
            if haskey(adict, a)
                adict[a] = intersect(adict[a], ingredients)
            else
                adict[a] = ingredients
            end
        end
    ...
    while true
        singletons = filter(p -> length(p[2]) == 1, adict)
        isempty(singletons) && break
        (agen, ing) = first(singletons)
        ingr = ing[1]
        sdict[ingr] = agen
        delete!(adict, agen)
        delete!(idict, ingr)
        for (alg, ingreds) in adict
            if ingr in ingreds
                adict[alg] = [x for x in ingreds if x != ingr]
            end
        end
        println("deleted $agen as $ingr")
    end
    println("part 1: ", sum(values(idict)))
end


part1()

part2() = println("part 2: ", join(sort(collect(keys(sdict)), lt = (x, y) -> sdict[x] < sdict[y]), ","))
